...
export class Rogue extends Hero{
  ...
  constructor(name: string, race: Race) {
    super(name, 300, 41, 49, 3, 'assets/rpg/hero/rogue.png', 'Rogue', race, 'color-rogue');
  }

  levelUp() {
    super.levelUp();
    this._lifePointMax = Math.round(this._lifePointMax + 30 * this.race.lifePoint);
    this._currentLifePoint = Math.round(this._currentLifePoint + 30 * this.race.lifePoint);
